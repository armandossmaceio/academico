...
public class RepositorioBancoTurmas implements IRepositorioTurmas{
	/**
	 * Instancia da classe. Ao ser instanciado, caso nao exista as tabalas criadas, o sistema as cria.
	 * @throws SQLException
	 */
	public RepositorioBancoTurmas() throws SQLException{
		//criar tabela turma
		...
		statement = ConnectionManager
				.reservaPreparedStatement(criarRelacao);
		statement.execute();
		statement.close();
	}
	/**
	 ...
	 * 
	 * @param Objeto turma a ser inserida
	 */
	 ...
	 * 
	 * @param o identificador da turma
	 */
	...
	public Turma buscarTurma(int identificador) throws TurmaNaoExistenteException, Exception {
		...
		while (result.next()) {
			t = new Turma(result.getString("departamento"),
					result.getString("disciplina"), result.getInt("carga_horaria"));
	 ...
	 * 
	 * @param o identificador da turma a ser buscada
	 */
	 ...
	 * 
	 * @param o identificador da turma a ser removida
	 * 
	 */
	...
	public List<Turma> listarTurmas() throws TurmasNaoCadastradasException, Exception {
		...
		while (result.next()) {
			Turma turma = new Turma(result.getString("departamento"),
					result.getString("disciplina"), result.getInt("carga_horaria"));
	 ...
	 * 
	 * @param O aluno e a turma que deseja realizar a relacao
	 */
	 ...
	 * 
	 * @param o aluno e a turma que deseja desfazer a relacao
	 */
	...
	public void removeAlunoTurma(Aluno aluno, Turma turma)
		...
		if(this.verificaExistenciaTurma(turma.getIdTurma())){
			if(dao.verificaExistenciaAluno(aluno.getMatricula())){
				...
				PreparedStatement statement = ConnectionManager
						.reservaPreparedStatement(delete);
	 ...
	 * 
	 * @param A turma que deseja obter tal informacao
	 ...
	 * 
	 * @param O professor e a turma que deseja criar a relacao
	 */
